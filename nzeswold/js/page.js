...
function Dsearch_adv() {
	Go(Dd('destoon_search').action.indexOf('/api/') != -1 ? DTPath + 'api/search.php?moduleid=' + Dd('destoon_moduleid').value :
		Dd('destoon_search').action);
}
...
function STip(w) {
	...
	$.post(AJPath, 'action=tipword&mid=' + searchid + '&word=' + w, function(data) {
		if (data) {
			Ds('search_tips');
			Dd('search_tips').innerHTML = data + '<label onclick="Dh(\'search_tips\');">' + L['search_tips_close'] +
				'&nbsp;&nbsp;</label>';
		} else {
			Dd('search_tips').innerHTML = '';
			Dh('search_tips');
...
function show_answer(u, i) {
	document.write('<iframe src="' + u + 'answer.php?itemid=' + i + '" name="destoon_answer" id="des' +
		'toon_answer" style="width:100%;height:0px;" scrolling="no" frameborder="0"></iframe>');
}

function show_task(s) {
	document.write('<script type="text/javascript" src="' + DTPath + 'api/task.js.php?' + s + '&refresh=' + Math.random() +
		'.js"></sc' + 'ript>');
}
var sell_n = 0;
...
function Dqrcode() {
	...
	if ($('#destoon_qrcode').length > 0) {
		if ($('#destoon_qrcode').html().length < 10) {
			...
			$('#destoon_qrcode').html(
				'<div style="text-align:right;color:#555555;font-size:16px;font-family:Verdana;font-weight:100;padding-right:6px;cursor:pointer;">x</div><img src="' +
				DTPath + 'api/qrcode.png.php?auth=' + encodeURIComponent(url) +
				'" width="140" height="140"/><div style="padding:10px 0;font-size:14px;font-weight:bold;color:#555555;">扫一扫，直接在手机上打开</div><div style="padding-bottom:20px;color:#999999;">推荐微信、QQ扫一扫等扫码工具</div>'
			);
...
function Dsearch2(i) {
	if (Dd('destoon_kw2').value.length < 1 || Dd('destoon_kw2').value == L['keyword_message2']) {
		Dd('destoon_kw2').value = '';
		window.setTimeout(function() {
			Dd('destoon_kw2').value = L['keyword_message2'];
		}, 500);
		return false;
	}
	if (i && Dd('destoon_search2').action.indexOf('/api/') == -1) {
		$('#destoon_moduleid2').remove();
		$('#destoon_spread2').remove();
	}
	return true;
}

function Dsearch_adv2() {
	Go(Dd('destoon_search2').action.indexOf('/api/') != -1 ? DTPath + 'api/search.php?moduleid=' + Dd('destoon_moduleid2')
		.value : Dd('destoon_search2').action);
}

function Dsearch_top2() {
	if (Dsearch(0)) {
		Dd('destoon_search2').action = DTPath + 'api/search.php';
		Dd('destoon_spread2').value = 1;
		Dd('destoon_search2').submit();
	}
}

function setModule2(i, n) {
	Dd('destoon_search2').action = DTPath + 'api/search.php';
	Dd('destoon_moduleid2').value = i;
	searchid = i;
	Dd('destoon_select2').value = n;
	$('#search_module2').fadeOut('fast');
	Dd('destoon_kw2').focus();
}

function setTip2(w) {
	Dh('search_tips2');
	Dd('destoon_kw2').value = w;
	Dd('destoon_search2').submit();
}
//浮动搜索-E
